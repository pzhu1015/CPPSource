...
// Description:
///////////////////////////////////////////////////////////////////
#ifndef SYSTEM_KAFKA_KAFKAHANDLEBASE_H
#define SYSTEM_KAFKA_KAFKAHANDLEBASE_H
...
#include "System/Kafka/GroupInformation.h"
#include <string>
#include <memory>
#include <chrono>
...
namespace System
{
	namespace Kafka
	{
		...
		*/
		class SYSTEM_API KafkaHandleBase {
		public:
			...
			* \return A topic
			*/
			Topic get_topic(const std::string& name, TopicConfig config);
		...
		private:
			...
			using TopicConfigurationMap = std::unordered_map<std::string, TopicConfig>;
			...
			GroupInformationList fetch_consumer_groups(const char* name,
				std::chrono::milliseconds timeout);
			void save_topic_config(const std::string& topic_name, TopicConfig config);
...
#endif // !SYSTEM_KAFKA_KAFKAHANDLEBASE_H
