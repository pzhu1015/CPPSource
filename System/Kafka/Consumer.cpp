	...
	namespace Kafka
	{
		...
		Consumer::~Consumer() {
			try {
				...
				CallbackInvoker<Config::ErrorCallback> error_cb("error", get_configuration().get_error_callback(), this);
				CallbackInvoker<Config::LogCallback> logger_cb("log", get_configuration().get_log_callback(), nullptr);
				if (error_cb) {
