...
namespace System
{
	namespace Kafka
	{
		...
		Topic KafkaHandleBase::get_topic(const string& name) {
			save_topic_config(name, TopicConfig{});
			return get_topic(name, nullptr);
		}

		Topic KafkaHandleBase::get_topic(const string& name, TopicConfig config) {
		...
		const Config& KafkaHandleBase::get_configuration() const {
			return config_;
		}
		...
		void KafkaHandleBase::save_topic_config(const string& topic_name, TopicConfig config) {
